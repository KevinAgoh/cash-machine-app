RSpec.describe CashMachine do
  ...
  it 'adds a product to the cart' do
    ...
    cash_machine.add_product_to_cart(product)
    expect(cash_machine.cart).to include(product)
  end
  ...
  it 'computes the total price of products in the cart' do
    ...
    cash_machine.add_product_to_cart(product1)
    cash_machine.add_product_to_cart(product2)
    expect(cash_machine.compute_total_price).to eq(8.11)
  end
  ...
  it 'applies a buy one, get one for free discount when you add a second tea to the cart' do
    ...
    cash_machine.add_product_to_cart(product1)
    cash_machine.add_product_to_cart(product1)
    expect(cash_machine.compute_total_price).to eq(3.11)
  end
  ...
  it 'reduces prices of strawberries to 4,50€ if there are 3 or more strawberries in the cart' do
    ...
    cash_machine.add_product_to_cart(product1)
    cash_machine.add_product_to_cart(product1)
    expect(cash_machine.compute_total_price).to eq(16.61)
  end
  ...
  it 'reduces prices of strawberries to 2/3 of the original price if there are 3 or more coffees in the cart' do
    ...
    cash_machine.add_product_to_cart(product1)
    cash_machine.add_product_to_cart(product3)
    cash_machine.add_product_to_cart(product2)
    cash_machine.add_product_to_cart(product3)
    cash_machine.add_product_to_cart(product3)
    expect(cash_machine.compute_total_price).to eq(30.57)
  end
end
